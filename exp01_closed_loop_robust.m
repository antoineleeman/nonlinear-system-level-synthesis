% File: exp01_closed_loop_robust.m
...
% % For the paper, we used: (requirement: installation of Gurobi)
% inexact_jac = true;
% solver_sqp = true; %false is ipopt
...
if ~solver_sqp
    ...
    if nlp_cons.inexact_jac
        [res, output] = nlp_cons.solve_sqp(inexact_jac,n_eq_1); % solve with inexact SQP variant
    else
...
save(getUniqueName('closed-loop.mat'));
